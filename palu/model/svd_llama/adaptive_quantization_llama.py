"""
Llama-specific sliding-window KV cache integration.
...
class PaluLlamaAttention(nn.Module):
    """
    Lightweight wrapper around a Llama attention module.
    ...
    def __init__(
        ...
        # Mirror key attributes expected from LlamaAttention / LlamaSdpaAttention
        ...
        **kwargs,
        ...
        if past_key_value is not None and self.run_sliding_step_if_needed is not None:
            try:
                ...
                logger.warning(f"[PaluLlamaAttention] run_sliding_step_if_needed failed: {e}")
        ...
        # We mimic standard LlamaAttention output signature: (attn_output, attn_weights, present_key_value)
        return attn_output, None, present_key_value

class LlamaAdaptiveQuantizationManager:
    """
    Manager for the sliding-window KV cache in Llama models.

    This class:
    1. Patches HeadwiseLowRankModule.forward() to automatically use QuantizedCache
    2. Wraps LlamaAttention so the sliding step fires at the right point in decode
    ...
    def __init__(
        ...
        """
        Initialize the sliding-window KV cache manager for a Llama model.

        Args:
            model: PaluLlamaForCausalLM model instance
    ...
    def _patch_projection_modules(self):
        ...
        if not hasattr(LlamaAdaptiveQuantizationManager, '_called_layers'):
            LlamaAdaptiveQuantizationManager._called_layers = set()
        if not hasattr(LlamaAdaptiveQuantizationManager, '_call_count'):
            LlamaAdaptiveQuantizationManager._call_count = {}
        patched_count = 0
            ...
            if isinstance(module, HeadwiseLowRankModule):
                ...
                if proj_type is not None:
                    ...
                    def patched_forward(hidden_states, cache_for_quantization=None, layer_name=None, proj_type=None, 
                        ...
                        if _captured_name not in LlamaAdaptiveQuantizationManager._called_layers:
                            LlamaAdaptiveQuantizationManager._called_layers.add(_captured_name)
    ...
    def _wrap_attention_layers(self):
        """
        Replace each layer's self-attention with PaluLlamaAttention, which
        ...
        """
        # Support both model.model.layers (e.g. PaluLlamaForCausalLM) and model.layers (e.g. LlamaModel / single "model" prefix)
        ...
        for layer_idx, layer in enumerate(layers_seq):
            ...
            if isinstance(attention, PaluLlamaAttention):
                continue

            # Only wrap modules that look like Llama attention (have q_proj/k_proj and rotary_emb)
    ...
    def _run_sliding_recent_quantize_step(self) -> None:
        ...
        # _maybe_run_sliding_step_before_append fires inside PaluLlamaAttention for
    ...
    def remove_hooks(self):
        ...
        if hasattr(LlamaAdaptiveQuantizationManager, '_call_count') and LlamaAdaptiveQuantizationManager._call_count:
            total_calls = sum(LlamaAdaptiveQuantizationManager._call_count.values())
            logger.info(f"Summary: {len(LlamaAdaptiveQuantizationManager._call_count)} unique layers called, {total_calls} total calls")
            # Only show first few layers
            for i, (cache_key, count) in enumerate(sorted(LlamaAdaptiveQuantizationManager._call_count.items())):
                if i < 3:
                    logger.info(f"  {cache_key}: {count} calls")
                elif i == 3:
                    logger.info(f"  ... (and {len(LlamaAdaptiveQuantizationManager._call_count) - 3} more layers)")
                    break
            # Reset for next run
            LlamaAdaptiveQuantizationManager._call_count = {}
            LlamaAdaptiveQuantizationManager._called_layers = set()
        ...
        for layer, original_attention in self.wrapped_attentions:
            try:
                if isinstance(layer.self_attn, PaluLlamaAttention):
